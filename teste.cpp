...
void accumulate(cv::Mat& input_img){
 ...
 Sobel(src_gray, dx, CV_16S, 1, 0, 3);

 Mat dy;

 dy.create( Size(input_img.cols, input_img.rows), CV_16SC1);

 Sobel(src_gray, dy, CV_16S, 0, 1, 3);
  ...
  for (int i=0; i<nc; ++i) {             
...

                           {
                            ...
                            float a = atan2((float)vy, (float)vx);
                                  // contour angle with respect to x axis

                            float phi = ((a > 0) ? a-pi_half : a+pi_half);

                                  // index associated with angle (0 index = -90 degrees)

                            int angleindex = (int)((phi+pi*0.5f)*inv_deltaphi);
                                                            // -90°angle and +90° has same effect
       ...
       // finds local maxima above a certain threashold
       ...
       // load vector pts with all points from the contour
             ...
             // read original template image and its worked-out contour
                 ...
                 Mat original_img = imread("template_original2.jpg", 1);

                 Mat input_img_gray;

                 input_img_gray.create( Size(original_img.cols, original_img.rows), CV_8UC1);

                 cvtColor(original_img, input_img_gray, COLOR_BGR2GRAY);

           //Mat template_img = imread("files\\contour_def.bmp", 1);

                 Mat template_img = imread("contour.jpg", 1);
             ...
             // Sobel(input_img_gray, dx, CV_16S, 1, 0, 5);   
              Canny(input_img_gray, dx, 50, 70, 3);
              ...
              //Sobel(input_img_gray, dy, CV_16S, 0, 1, 5);
              Canny(input_img_gray, dy, 50, 70, 3);
       ...
       //c = 'r'; // run algorithm
             ...
             //Mat detect_img = imread("files\\Img_01.png", 1);
                         ...
                         Mat detect_img = imread("template_original2.jpg", 1);
